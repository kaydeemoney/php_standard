...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)){
        ...
        }else if ( $utility_class_call->input_is_invalid($name) || $utility_class_call->input_is_invalid($duration) || $utility_class_call->input_is_invalid($commission) ||   $utility_class_call->input_is_invalid($category_tid) || $utility_class_call->input_is_invalid($description)||$utility_class_call->input_is_invalid($data_tid) ) {
        ...
        } else if (!$utility_class_call->input_is_invalid($images) && count($images['name']) > 1) {
            $api_status_code_class_call->respondBadRequest(API_User_Response::$onFileAtATime);
        }else{ 
            $galleryimaglinks='';
            $bannerimaglinks='';
            $getoldimges= $db_call_class->selectRows("services",'banner_image, gallery', [[
                ['column' => 'trackid', 'operator' => '=', 'value' => $data_tid]]
            ]);

            if(!$utility_class_call->input_is_invalid($getoldimges)){
                $galleryimaglinks=$getoldimges[0]['gallery'];
                $bannerimaglinks=$getoldimges[0]['banner_image'];
            }
            ...
            $imagesarevalid = 0;//banner
            $imagesarevalid1 = 0;//gallery
            ...
            $insertData= $db_call_class->updateRows("services",["cat_tid" => $category_tid, 'name' => $name,'commission' => $commission, 'min_duration' => $duration, 'banner_image' => $bannerimaglinks, 'status' => $status, 'feature_front_page' => $featured, 'description' => $description, 'gallery' => $galleryimaglinks], [[
                ['column' => 'trackid', 'operator' => '=', 'value' => $data_tid]]
            ]);
