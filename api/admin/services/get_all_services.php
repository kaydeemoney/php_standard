...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)){
            ...
            //count how many rows
            $total_numRow =$db_call_class->checkIfRowExistAndCount("services", $whereclause);
            
            // get the dataf
            $getAllData= $db_call_class->selectRows("services", "trackid, cat_tid, name, commission, status, description, min_duration, 	feature_front_page, banner_image, gallery AS imagelinks", $whereclause, ['limit' => $noperpage,'orderBy' => 'id','orderDirection' => 'DESC', 'pageno' => $page_no]);
               ...
               // Iterate through the data array
                foreach ($getAllData as &$item) {
                    if (isset($item['banner_image'])) {
                        $item['banner_image']= $base_url.str_replace(',','',$item['banner_image']);
                    }
