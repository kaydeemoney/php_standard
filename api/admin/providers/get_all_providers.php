...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try {
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)) {
            ...
            // array_push($whereclause, [['column' => 'users.am_a_provider', 'operator' => '=', 'value' => 1]]);
            if (!$utility_class_call->input_is_invalid($sort) && $sort != 0) {
                $sortir = $sort == 2 ? 0 : $sort;
                array_push($whereclause, [['column' => 'users.account_verified', 'operator' => '=', 'value' => $sortir]]);
            }
            ...
            // get the dataf
            $getAllData = $db_call_class->selectRows("users", "users.documentlinks, users.bank_holder_name, users.bank_name, users.bank_route_no, users.bank_address, users.bank_acc_no, users.id,users.trackid, users.fname, users.lname, users.username, 
            CONCAT('$baseurl', users.profile_pic) AS profile_pic, users.phoneno, users.status, users.email, users.address1, users.about, userwallet.walletbal, account_verified, users.service_tid, services.name as service_name", $whereclause, [ 'joins' => [[
            'type' => 'LEFT',
            'table' => 'userwallet',
            ...
            if ($utility_class_call->input_is_invalid($getAllData)||$getAllData[0]['id']==null) {
                ...
                foreach ($getAllData as &$item) {
                    $provider_tid=$item['trackid'];
                    $item['total_earned']='0';
                    $item['total_reveiw']=0;
                    $item['total_completed_book']=0;
                    $item['reveiw_ratings']=0;
                    $item['bookings']=0;
                    $item['totalbookings']=0;
                    $totalpaidis=$db_call_class->selectRows("bookings","SUM(provider_fee) AS total,COUNT(*) as total_profit",[[
                    ['column' => 'status', 'operator' => '=', 'value' =>1],['column' => 'provider_tid', 'operator' => '=', 'value' =>$provider_tid]]], []);

                    if ($utility_class_call->input_is_invalid($totalpaidis)){
                        $item['total_earned'] =$totalpaidis[0]['total'];
                        $item['total_completed_book']=$totalpaidis[0]['total_profit'];
                    }
                    $totalpaidis=$db_call_class->selectRows("bookings","SUM(amt_paid) AS total,COUNT(*) as total_profit",[[
                        ['column' => 'status', 'operator' => '=', 'value' =>1],['column' => 'user_tid', 'operator' => '=', 'value' =>$provider_tid]]], []);
    
                        if ($utility_class_call->input_is_invalid($totalpaidis)){
                            $item['bookings'] =$totalpaidis[0]['total'];
                            $item['totalbookings']=$totalpaidis[0]['total_profit'];
                        }


                    $totalpaidis=$db_call_class->selectRows(" reviews","SUM(rating) AS total_rate,COUNT(*) as total", [[['column' => 'provider_tid', 'operator' => '=', 'value' =>$provider_tid]]], []);

                    if ($utility_class_call->input_is_invalid($totalpaidis)){
                        $item['total_reveiw']=$totalpaidis[0]['total'];
                        $item['reveiw_ratings']=$totalpaidis[0]['total_rate']/$totalpaidis[0]['total'];
                    }
                    // documents
                    if (isset($item['documentlinks'])) {
                        // Split the documentlinks string by comma
                        $image_urls = explode(',', $item['documentlinks']);
                       ...
                       // Remove any empty elements in case the split left an empty string
                        ...
                        $item['documentlinks'] = $full_urls;
                    }

                    if (isset($item['account_verified'])) {
                        $item['account_verified_text']='Pending';
                        if($item['account_verified']==0){
                            $item['account_verified_text']='Pending';
                        }else if($item['account_verified']==1){
                            $item['account_verified_text']='Approved';
                        }else if($item['account_verified']==2){
                            $item['account_verified_text']='Rejected';
                        }else if($item['account_verified']==3){
                            $item['account_verified_text']='Processing';
                        }
                    }
                }
