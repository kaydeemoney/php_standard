    ...
    try {
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)) {
            ...
            if ( !$utility_class_call->input_is_invalid($sort) && $sort != 0 ) {
                $sortir = $sort == 2 ? 0 : 1;
                array_push($whereclause, [['column' => 'status', 'operator' => '=', 'value' =>$sortir ]]);
            }
            ...
            //count how many rows
            $total_numRow = $db_call_class->checkIfRowExistAndCount("admin", $whereclause);
            // get the dataf
            $getAllData = $db_call_class->selectRows("admin", "trackid, name, userlevel, profile_updated, status, email", $whereclause, ['limit' => $noperpage, 'orderBy' => 'id', 'orderDirection' => 'DESC', 'pageno' => $page_no]);
