...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)){
        ...
        }else if ( $utility_class_call->input_is_invalid($name) || $utility_class_call->input_is_invalid($data_tid) || $utility_class_call->input_is_invalid($email) || $utility_class_call->input_is_invalid($level)) {
            ...
            if (!$utility_class_call->input_is_invalid($password)) {
                $password = $utility_class_call->Password_encrypt($password);
                $passaray = ['password' => $password];
            }
            
            $updateData = array_merge( ['name' => $name, 'email' => $email, 'userlevel' => $level, 'status' => $status], $passaray );
            $insertData = $db_call_class->updateRows("admin", $updateData, [[
                ['column' => 'trackid', 'operator' => '=', 'value' => $data_tid]]
            ]);
