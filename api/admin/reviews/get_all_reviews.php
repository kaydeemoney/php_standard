    ...
    try {
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)) {
            ...
            if (!$utility_class_call->input_is_invalid(($sort)) && $sort != 0) {
                $sortir = $sort == 2 ? 0 : 1;
                array_push($whereclause, [['column' => 'reviews.status', 'operator' => '=', 'value' => $sortir]]);
            }

            if (!$utility_class_call->input_is_invalid(($user_tid))) {
                array_push($whereclause, [['column' => 'reviews.user_tid', 'operator' => '=', 'value' => $user_tid]]);
            }

            if (!$utility_class_call->input_is_invalid(($provider_tid))) {
                array_push($whereclause, [['column' => 'reviews.provider_tid', 'operator' => '=', 'value' => $provider_tid]]);
            }
            ...
            if (!$utility_class_call->input_is_invalid(($search))) {
                ...
                ['column' => 'reviews.trackid', 'operator' => 'LIKE', 'value' => $search],
                'operator' => 'OR']);
            }

            //count how many rows
            $baseurl=Constants::BASE_URL.'/';
            $total_numRow = $db_call_class->checkIfRowExistAndCount("reviews", $whereclause, [], $topLevelOperator);
            // get the dataf
            $getAllData = $db_call_class->selectRows("reviews", "reviews.trackid, reviews.rating, reviews.review, services.name as service_name, uu.lname as ulname, up.fname as pfname", $whereclause, [ 'joins' => [[
            'type' => 'LEFT',
            'table' => 'services',
            'condition' => 'reviews.service_tid = services.trackid' ], [
                'type' => 'LEFT',
                'table' => 'users uu',
                'condition' => 'reviews.user_tid = uu.trackid'], [
            'type' => 'LEFT',
            'table' => 'users up',
            'condition' => 'reviews.provider_tid = up.trackid']],
            'limit' => $noperpage, 'orderBy' => 'reviews.id', 'orderDirection' => 'DESC', 'pageno' => $page_no], $topLevelOperator);

            if ($utility_class_call->input_is_invalid($getAllData)||$getAllData[0]['trackid']==null) {
