...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try {
        ...
        $location = isset($_POST['location']) ? $utility_class_call->clean_user_data($_POST['location']) : '';
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)) {
        ...
        } else if ($utility_class_call->input_is_invalid($name) || $utility_class_call->input_is_invalid($location) || $utility_class_call->input_is_invalid($description) || $utility_class_call->input_is_invalid($images)) {
            ...
            $imagelocation = "$folderPath/assets/images/sliders/";
            $imagelocationName = "/assets/images/sliders/";
            ...
            if($imagesarevalid==1){
                $imagetoSave =$utility_class_call->uploadImage($images, $imagelocation, $tmpimagelocation, $imagelocationName, $name);
                $insertData = $db_call_class->insertRow("sliders", ["title" => $name, "urltogo" => $urltogo, 'location' => $location, 'status' => $status, 'imagelinks' => $imagetoSave, 'description' => $description]);
