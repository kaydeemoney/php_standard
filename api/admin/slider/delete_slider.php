...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try {
        ...
        $getSliderData = $db_call_class->selectRows("sliders", "imagelinks", [[
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)) {
            ...
            $sliderimages = $getSliderData[0]['imagelinks'];
            $getallimages = explode(",", $sliderimages);
            for($i=0; $i < count($getallimages); $i++){
                if(!empty($getallimages[$i])){
                    $assetspath='/assets/images/sliders/';
            ...
            $insertData = $db_call_class->deleteRows( "sliders", [ [
                ['column' => 'id', 'operator' => '=', 'value' => $data_tid]]
            ]);
