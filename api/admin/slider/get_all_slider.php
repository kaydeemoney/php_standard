    ...
    try{
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)){
            ...
            if(!$utility_class_call->input_is_invalid(($search))){
                ...
                ['column' =>'urltogo', 'operator' =>'LIKE', 'value' =>$search],
                'operator'=>'OR']];
            }

            //count how many rows
            $total_numRow =$db_call_class->checkIfRowExistAndCount("sliders",$whereclause);
            // get the dataf
            // CONCAT('$baseurl', flag_url) AS flag_url
            $getAllData= $db_call_class->selectRows("sliders", "id, title, location, status, description, imagelinks, urltogo", $whereclause,['limit'=>$noperpage,'orderBy' => 'id','orderDirection' => 'DESC', 'pageno' => $page_no]);
