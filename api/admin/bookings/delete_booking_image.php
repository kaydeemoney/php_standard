...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{
        ...
        $img_link = isset($_POST['image_link']) ? $utility_class_call->clean_user_data($_POST['image_link'], 1) : '';
        $getSliderData = $db_call_class->selectRows("bookings", "imageslink", [[
            ['column' => 'trackid', 'operator' => '=', 'value' =>$data_tid]]
        ]);

        if ( $utility_class_call->input_is_invalid($getuserattached) ){
            $api_status_code_class_call->respondUnauthorized();
        }else if ( $utility_class_call->input_is_invalid($data_tid) || $utility_class_call->input_is_invalid($img_link) ) { 
            ...
            $img_link = str_replace($base_url, '', $img_link);
            $galleryimages = $getSliderData[0]['imageslink'];
            $getallimages = explode(",", $galleryimages);
            $key = array_search($img_link, $getallimages);

            if ($key !== false) {
                unset($getallimages[$key]);
            }

            $assetspath = '/assets/images/bookings/';
            // /assets/images/sliders/taskcolony665bf151eea224.67851064loveddd.webp,
            $filename = str_replace($assetspath, '', $img_link);
            ...
            $galleryimaglinks = implode(",", $getallimages);
            $insertData = $db_call_class->updateRows("bookings", ['imageslink' => $galleryimaglinks], [[
                ['column' => 'trackid', 'operator' => '=', 'value' => $data_tid]]
            ]);
