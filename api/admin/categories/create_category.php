...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{
        ...
        $icon = isset($_POST['icon']) ? $utility_class_call->clean_user_data($_POST['icon']) : '';
        $status = isset($_POST['status']) ? $utility_class_call->clean_user_data($_POST['status']) : '';
        $total_numRow =$db_call_class->checkIfRowExistAndCount("categories", [[
            ['column' =>'name', 'operator' =>'=', 'value' =>$name]]
        ]);

        if ($utility_class_call->input_is_invalid($getuserattached)){
            $api_status_code_class_call->respondUnauthorized();
        }else if ( $utility_class_call->input_is_invalid($name) || $utility_class_call->input_is_invalid($icon)) {
            ...
            $insertData = $db_call_class->insertRow("categories", ["name" => $name, 'trackid' => $trackid, 'icon' => $icon, "status" => $status]);
