    ...
    try{    
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)){
            ...
            if(!$utility_class_call->input_is_invalid(($search))){
                ...
                ['column' => 'icon', 'operator' => 'LIKE', 'value' => $search],
                'operator' => 'OR']];
            }

            //count how many rows
            $total_numRow = $db_call_class->checkIfRowExistAndCount("categories", $whereclause);
            // get the dataf
            $getAllData = $db_call_class->selectRows("categories", "trackid, name, icon, status", $whereclause, ['limit' => $noperpage,'orderBy' => 'id', 'orderDirection' => 'DESC', 'pageno' => $page_no]);
