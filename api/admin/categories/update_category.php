...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{
        ...
        }else if ( $utility_class_call->input_is_invalid($name) || $utility_class_call->input_is_invalid($icon)|| $utility_class_call->input_is_invalid($data_tid)) { 
            $api_status_code_class_call->respondBadRequest(API_User_Response::$request_body_invalid);
        }else{   
            $insertData= $db_call_class->updateRows("categories", ["name" => $name, 'icon' => $icon, "status" => $status], [[
