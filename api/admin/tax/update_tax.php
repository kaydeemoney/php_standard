...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{
        ...
        $data_tid = isset($_POST['tax_tid']) ? $utility_class_call->clean_user_data($_POST['tax_tid']) : '';
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)){
            ...
            $insertData= $db_call_class->updateRows("countries_cities", ["name" => $name, 'time_zone_name' => ' ', 'state_tid' => 'HJDS', 'commission' => $commission, 'status' => $status], [[
                ['column' => 'trackid', 'operator' => '=', 'value' => $data_tid]]
            ]);
