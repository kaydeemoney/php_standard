    ...
    try{
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)){
            ...
            if(!$utility_class_call->input_is_invalid($search)){
                ...
                ['column' =>'coupoun_code', 'operator' => 'LIKE', 'value' => $search],
                'operator'=>'OR']];
            }

            //count how many rows
            $total_numRow = $db_call_class->checkIfRowExistAndCount("coupons", $whereclause);
            // get the dataf
            $getAllData = $db_call_class->selectRows("coupons", "trackid, name, type, coupon_value, expiredate, services_id, status,display_on_app, description, coupoun_code", $whereclause, ['limit' => $noperpage, 'orderBy' => 'id', 'orderDirection' => 'DESC', 'pageno' => $page_no]);
            ...
            if ($utility_class_call->input_is_invalid($getAllData)) {
                ...
                foreach ($getAllData as &$item) {
                    if (isset($item['status'])) {
                        $item['statustext'] = $item['status'] == 1 ? "Active" : "In Active";
                    }

                    if (isset($item['display_on_app'])) {
                        $item['displaytext'] = $item['display_on_app'] == 1 ? "YES" : "NO";
                    }

                    if (isset($item['type'])) {
                        $item['typetext'] = $item['type'] == 1 ? "Percent" : "Pay Off";
                    }

                    if (isset($item['services_id'])) {
                        if($item['services_id'] != 0){
                            $servicesnames = [];
                            $seprataeids = explode(",", $item['services_id']);
                            $item['services_id'] = $seprataeids;

                            for($i = 0; $i < count($seprataeids); $i++){
                                $serviceidis = $seprataeids[$i];
                                $getInnerData = $db_call_class->selectRows("services", "name", [[
                                    ['column' => 'trackid', 'operator' => '=', 'value' => $serviceidis]]],
                                    ['limit' => 1, 'orderBy' => 'id', 'orderDirection' => 'DESC', 'pageno' => 1]
                                );

                                if (!$utility_class_call->input_is_invalid($getInnerData)) {
                                    array_push($servicesnames, $getInnerData[0]['name']);
                                }
                            }

                            $item['services_name'] = implode(", ", $servicesnames);
                        }else{
                            $item['services_name'] = "All Services";
                        }
                    }
                }
