...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{
        ...
        $coupon_tid = isset($_POST['coupon_tid']) ? $utility_class_call->clean_user_data($_POST['coupon_tid']) : '';
        $type = isset($_POST['type']) ? $utility_class_call->clean_user_data($_POST['type']) : '';
        $value = isset($_POST['value']) ? $utility_class_call->clean_user_data($_POST['value']) : '';
        $expiredate = isset($_POST['expiredate']) ? $utility_class_call->clean_user_data($_POST['expiredate'], 1) : '';
        $status = isset($_POST['status']) ? $utility_class_call->clean_user_data($_POST['status']) : '';
        $displayonapp = isset($_POST['displayonapp']) ? $utility_class_call->clean_user_data($_POST['displayonapp']) : '';
        $description = isset($_POST['description']) ? $utility_class_call->clean_user_data($_POST['description']) : '';
        $services = isset($_POST['services']) ? $utility_class_call->clean_user_data($_POST['services'], 1) : ''; // seprated with comma, 0 means all

        if ($utility_class_call->input_is_invalid($getuserattached)){
            $api_status_code_class_call->respondUnauthorized();
        }else if ( $utility_class_call->input_is_invalid($name) || $utility_class_call->input_is_invalid($type) || $utility_class_call->input_is_invalid($value) || $utility_class_call->input_is_invalid($expiredate) || $utility_class_call->input_is_invalid($description) || $utility_class_call->input_is_invalid($coupon_tid)) { 
            $api_status_code_class_call->respondBadRequest(API_User_Response::$request_body_invalid);
        }else{   
            $insertData = $db_call_class->updateRows("coupons", ["name" => $name, 'coupon_value' => $value, 'expiredate' => $expiredate, 'services_id' => $services, 'status' => $status, 'display_on_app' => $displayonapp, 'description' => $description, 'type' => $type], [[ ['column' => 'trackid', 'operator' => '=', 'value' => $coupon_tid] ]]);
