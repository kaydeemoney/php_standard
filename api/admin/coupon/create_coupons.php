...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{
        ...
        if ($utility_class_call->input_is_invalid($getuserattached)){
        ...
        }else if ( $utility_class_call->input_is_invalid($name) || $utility_class_call->input_is_invalid($type) || $utility_class_call->input_is_invalid($value) ||   $utility_class_call->input_is_invalid($expiredate) || $utility_class_call->input_is_invalid($description) ) {
            ...
            $coponcode = $db_call_class->createUniqueRandomStringForATableCol(6, "coupons", "coupoun_code", "", true, true, false);    
            $insertData = $db_call_class->insertRow("coupons", ["name" => $name, 'trackid' => $trackid, 'coupoun_code' => $coponcode,'coupon_value' => $value, 'expiredate' => $expiredate, 'services_id' => $services, 'status' => $status, 'display_on_app' => $displayonapp, 'description' => $description, 'type' => $type]);
