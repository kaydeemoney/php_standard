...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try {
        ...
        $getuserattached = $db_call_class->selectRows("admin", "trackid, name, userlevel, profile_updated, status, email, CONCAT('$baseurl', profile_image) As profile_image", [[ ['column' => 'adminpubkey', 'operator' => '=', 'value' => $user_pubkey]] ]);
       
        if ($utility_class_call->input_is_invalid($getuserattached)) {
            $api_status_code_class_call->respondUnauthorized();
        } else {
            $getAllData=$getuserattached[0];

            if ($utility_class_call->input_is_invalid($getAllData)) {
                $text = API_User_Response::$data_not_found;
                $maindata['data'] = [];
            } else {
                    if (isset($getAllData['userlevel'])) {
                        $getAllData['userlevel_name'] = $getAllData['userlevel'] == 1 ? 'Super Admin' : 'Base Admin';
                    }
