...
if (getenv('REQUEST_METHOD') == $apimethod) {
    try{    
        ...
        }else if ( $utility_class_call->input_is_invalid($data_tid) || $utility_class_call->input_is_invalid($review)) {
            $api_status_code_class_call->respondBadRequest(API_User_Response::$request_body_invalid);
        }else{   
            $insertData= $db_call_class->updateRows("reviews", ['review' => $review], [[
            ['column' =>'trackid', 'operator' =>'=', 'value' => $data_tid],
            ['column' =>'user_tid', 'operator' =>'=', 'value' => $getuserattached[0]['trackid']],
            'operator' => "AND"]]);
